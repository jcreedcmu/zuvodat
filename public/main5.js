var DP$0 = Object.defineProperty;SCALE = 3;
BOARD_SIZE = 169;
AVATAR_OFF = 151;
AVATAR_SIZE = 17;
BGCOLOR = '#ddc492';
...
var View = (function(){"use strict";
  function View(state) {
    this.state = state;
  }DP$0(View, "prototype", {"configurable": false, "enumerable": false, "writable": false});

  View.prototype.set_state = function(state) {
    this.state = state;
  }

  View.prototype.render = function(d) {
    var st = this.state;
    var g = ld.img.game;
    ...
    if (!hit_test) {
      ...
      // victory cleanup
      if (st.victory) {
	_.each([0,1], function(player ) {
	  _.each([0,1], function(side ) {
    ...
    var blit = function(src, dst, size, data)  {
      if (hit_test) {
    ...
    var draw_stones = function(player, side)  {
      return function(num, ix)  {
	...
	var data = {player: player, side: side, ix: ix, action: 'add', active: !inactive};
	...
	data = {player: player, side: side, ix: ix, action: 'kick', active: active};
  ...
  View.prototype.do_mouse = function(e) {
    var st = this.state;
    var p = relpos(e, c);
    p.hit_test = true;
    try {
...
var Lookahead = (function(){"use strict";

  function Lookahead(opts) {
    this.depth = 3;
    this.width = 1;
    if (opts != null)
      _.extend(this, opts);
    console.log(this.width);
  }DP$0(Lookahead, "prototype", {"configurable": false, "enumerable": false, "writable": false});

  Lookahead.prototype.cand_moves = function(st) {
    var rv = [];
	...
	rv.push({player: player, action: 'kick', side: sd, ix: bix});
      }
      for (var bbix = bix - this.width; bbix <= bix + this.width; bbix++) {
	if (bbix > 0 && bbix < MAX_STONES-1 && st.stones[player][sd][bbix] < 11)
	  rv.push({player: player, action: 'add', side: sd, ix: bbix});
      }
    }
    return rv;
  }

  Lookahead.prototype.eval = function(st, depth) {var this$0 = this;
    var player = st.cur_player;
    if (st.victory) {
      return -1e9;
    }
	...
	5 * (ballgood.x + ballgood.y);
      ...
      return -_.min(_.map(moves, function(move ) {return this$0.eval(st.step(move), depth-1)}));
    }
  }

  Lookahead.prototype.best_move = function(st) {var this$0 = this;
    var moves = this.cand_moves(st);
    return _.min(moves, function(move ) {return this$0.eval(st.step(move), this$0.depth)});
  }
;return Lookahead;})();

var Simple = (function(){"use strict";function Simple() {}DP$0(Simple, "prototype", {"configurable": false, "enumerable": false, "writable": false});

  Simple.prototype.best_move = function(state) {
	...
	return {player: player, action: 'kick', side: sd, ix: state.ball[dir]};
      }
    }

    for(var sd$0 = 0; sd$0 < 2; sd$0++) {
      var dir$0 = ['x', 'y'][sd$0];
      if (obstone[0][sd$0] > 0 && obstone[1][sd$0] > 0) {
	return {player: player, action: 'kick', side: sd$0, ix: state.ball[dir$0]};
      }
    }
	...
	return {player: player, action: 'kick', side: side, ix: state.ball[sxy]};
      }
      if (state.stones[player][side][state.ball[sxy]] >= 2 && Math.random() < 0.8) {
	return {player: player, action: 'kick', side: side, ix: state.ball[sxy]};
      }
      return {player: player, action: 'add', side: side, ix: state.ball[sxy]};
    }
